 ...
 * 		pMap(in): 整个地图
 * 		nIterations(in): 迭代次数
 * 		pbStopFlag(in): 停止标志量,表示是否该强制停止全局优化
 *		 nLoopKF(in): 找到的回环帧的id
 * 		bRobust(in): 是否需要鲁棒性内核
 * 	
 ******/
                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    // Set KeyFrame vertices  设置关键帧位姿节点
    ...
    // Set MapPoint vertices  设置地图点三维点坐标节点
    for(size_t i=0; i<vpMP.size(); i++)
    {
       ...
       const map<KeyFrame*,size_t> observations = pMP->GetObservations();
        ...
        int nEdges = 0;
        //SET EDGES  设置边  两端分别是地图点三维坐标和观察到该三维点坐标的关键帧
        ...
        if(nEdges==0)  // 如果该地图点没有任何的边,则从图优化中删除该地图点
        {
...
// 优化的是局部图   局部图中包括一种地图点:共视关键帧的地图点       两种关键帧:  1 共视关键帧 2 可以看到共视关键帧地图点的其他非共视关键帧
// 优化的是共视关键帧位姿和共视关键帧的地图点坐标
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap)
{    
    // Local KeyFrames: First Breath Search from Current Keyframe   
    // 步骤1: 初步的局部关键帧   当前帧的共视关键帧
    ...
    // 步骤3: 修正关键帧,可以看到局部地图点但不在局部关键帧中 注意图优化的关键帧节点分为两部分vNeighKFs和lFixedCameras都有
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(pKFi->mnBALocalForKF!=pKF->mnId && pKFi->mnBAFixedForKF!=pKF->mnId)   // 此条件是为了将局部关键帧排除在外
            {                
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        vSE3->setFixed(pKFi->mnId==0);  // 只有初始关键帧是固定位姿单位矩阵的 其余的关键帧位姿都可以根据图优化位姿进行调整
        optimizer.addVertex(vSE3);
    ...
    // 步骤6: 设置观察到局部地图点的非局部关键帧集作为图优化的帧节点
    ...
    if(pbStopFlag)
        if(*pbStopFlag)
            return;
    // 步骤9: 优化一次,筛选内点后再优化一次进一步筛选内点,得到最终的结果
    optimizer.initializeOptimization();
    ...
    // Check inlier observations  检测内点
    ...
    // Optimize again without the outliers   去除外点之后进行进一步的优化
    ...
    // Check inlier observations       再次检测内点
    for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
    {
        g2o::EdgeSE3ProjectXYZ* e = vpEdgesMono[i];
        MapPoint* pMP = vpMapPointEdgeMono[i];
    ...
    // Recover optimized data   恢复优化数据  包括关键帧的位姿和地图点的三维坐标
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections, const bool &bFixScale)
{
    ...
    // Set Loop edges   设置回环边  边的两端分别是回环的两个端点,回环的初始端点是一个关键帧集合,末端点与该关键帧集合形成若干的边

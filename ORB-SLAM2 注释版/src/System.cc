 ...
 *                        （3）创建关键帧数据集
 *                        （4）创建全局地图
 *                        （5）初始化观测器（关键帧观测器、地图观测器）、图像追踪器、局部地图构建器、回环检测器、
 *                        （6）四个线程初始化（追踪线程（主线程）、局部地图构建线程、回环检测线程、观测器线程）
 *                        （7）四个线程之间建立联系
 *     函数参数介绍：
 *                          strVocFile：指向字典的路径
...
cv::Mat System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp)
{
  ...
  /*******************************************************************************
 *     函数属性：类System的成员函数函数TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp)
 *     函数功能：（1）检测传感器类型
 ...
 *                          depthmap：深度图像
 *                          timestamp：该帧数据获取时间戳
 *     备注：将Kinect获取的图像加入SLAM系统中，并开启追踪线程
 * 
 ******************************************************************************/
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp)
{
    ...
    unique_lock<mutex> lock2(mMutexState);
    mTrackingState = mpTracker->mState;     //给当前追踪线程状态赋值
    mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;   //加入地图点
    mTrackedKeyPointsUn = mpTracker->mCurrentFrame.mvKeysUn;     //加入关键点
    return Tcw;
}
...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    // Check mode change
    {
        ...
        {
            mpLocalMapper->RequestStop(); // 局部地图请求停止

            // Wait until Local Mapping has effectively stopped   等待局部地图被停止
            while(!mpLocalMapper->isStopped())
            {
                usleep(1000);
            }
        ...
        if(mbDeactivateLocalizationMode)//取消仅定位模式
        {
            mpTracker->InformOnlyTracking(false);
            mpLocalMapper->Release();
            mbDeactivateLocalizationMode = false;
...
void System::SaveTrajectoryTUM(const string &filename)
{
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        vector<float> q = Converter::toQuaternion(Rwc);//旋转矩阵用 四元数表示
